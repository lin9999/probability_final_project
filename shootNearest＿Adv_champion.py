...
def survivalRateList(acc, alive):
    ret = [0.0]
    acc = np.array(acc)
...
def shoot_next_1(acc, alive, you):
    ...
    for i in range(1, len(alive)):
        if alive[i] and i != you:
            if abs(acc[i] - acc[you]) < minDistance:
                minDistance = abs(acc[i] - acc[you])
                index = i
...
def shoot_next_2(acc, alive, turn):
    ...
    index = 0
    for i in range(1, len(alive)):
        if alive[i] and i != turn:
            if abs(acc[i] - acc[turn]) < minDistance:
                minDistance = abs(acc[i] - acc[turn])
                index = i
